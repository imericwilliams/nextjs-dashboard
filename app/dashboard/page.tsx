...
export default async function Page() {
  ...
  const latestInvoices = await fetchLatestInvoices(); // wait for fetchRevenue() to finish due to *REQUEST WATERFALL (Sequential data fetching)
  const {
    numberOfInvoices,
    numberOfCustomers,
    totalPaidInvoices,
    totalPendingInvoices,
  } = await fetchCardData(); // wait for fetchLatestInvoices() to finish due to *REQUEST WATERFALL (Sequential data fetching)
